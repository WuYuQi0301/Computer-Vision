...
void Delaunay::MakeTriangle(int n1, int n2, int n3)
{
	...
	for (int i = 0; i<3; i++)
	{
		...
		{
			if (newTriangle.s[i].left == m_Edges[j].left 
			 && newTriangle.s[i].right == m_Edges[j].right
			 && m_Edges[j].count != -1) 
			{
				flag = 0;
				m_Edges[j].count += 1;
...
bool Delaunay::AddPoint(double xx, double yy, double zz)
{
	...
	for (int i = 0; i<(int)BoundEdges.size(); i++)
	{
		if (PtSize - 1<BoundEdges[i].left) 
			MakeTriangle(PtSize - 1, BoundEdges[i].left, BoundEdges[i].right);
		else if (PtSize - 1>BoundEdges[i].left && PtSize - 1<BoundEdges[i].right) 
			MakeTriangle(BoundEdges[i].left, PtSize - 1, BoundEdges[i].right);
